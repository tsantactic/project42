...
static void	manage_fractal(t_complex *z, t_complex *c, t_fractal *fractal)
{
	...
	else if (!ft_strncmp(fractal->name, "tricorn", 8))
	{
		c->x = z->x;
		c->y = z->y;
	}
...
static void	draw_pixel(int x, int y, t_fractal *fractal)
{
	...
	pixel_position(x, y, &fractal->img, WHITE);
}
